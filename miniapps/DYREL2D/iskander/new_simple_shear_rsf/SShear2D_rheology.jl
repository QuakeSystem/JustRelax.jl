...
function init_rheology_nonNewtonian_rsf()
    # RSF run uses elastic + viscous branches only (plasticity disabled).
    disl_wet_olivine = SetDislocationCreep(Dislocation.wet_olivine1_Hirth_2003)
    diff_wet_olivine = SetDiffusionCreep(Diffusion.wet_olivine_Hirth_2003)
    el = ConstantElasticity(; G = 40.0e9, ν = 0.25)
    lithosphere_rheology = CompositeRheology((el, disl_wet_olivine, diff_wet_olivine))
...
function init_rheologiesnew(lithosphere_rheology)
...
function init_rsf_params_ratestate_simple_shear(di_min; V0_model = 4.0e-9)
    # RSF parameters follow LaMEM-style names, phase-wise except V0_model.
...
function init_rsf_params_simple_shear(di_min)
    ...
    return (
        ...
        mu_d = (0.5, 0.15, 0.7, 0.0),          # dynamic friction coefficient
